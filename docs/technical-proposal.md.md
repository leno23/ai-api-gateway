...
- **HTTP API 契约**：运行时提供 `GET /openapi.yaml`（OpenAPI 3），与实现同源 spec 位于 `backend/internal/openapi/spec.yaml`；详见仓库 `README.md`。
...
仓库为 **monorepo**：Go 网关在 `backend/`（`go.mod` 与 `go` 命令均在此目录），管理控制台在 `frontend/`（实现阶段落地，见 OpenSpec `gateway-admin-console-frontend`）。

```
ai-api-gateway/
├── backend/                         # Go 网关（cd backend 后 go build / go test / go run）
│   ├── cmd/server/main.go           # 程序入口
│   ├── internal/                    # config、handler(relay/admin)、middleware、openapi、routing、service、store 等
│   ├── migrations/                  # SQL 迁移
│   ├── go.mod
│   ├── go.sum
│   └── .env.example
├── frontend/                        # 管理控制台（Next.js 等，规划见 openspec）
├── docs/
├── openspec/
├── docker-compose.yml               # 本地 Postgres / Redis（仓库根）
└── README.md
```

下列为后端 **包级目标结构**（路径均相对于 `backend/`，与当前 `internal/` 演进可能略有差异，以源码为准）：
...
│   └── admin/                       # 管理 API
├── adapter/                         # 各上游适配
├── router/、routing/                # 路由与负载
├── model/、repository/、service/
└── openapi/                         # OpenAPI spec 嵌入与 GET /openapi.yaml
```
